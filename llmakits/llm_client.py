...
class BaseClient :
    ...
    def send_message( self, messages, message_info = None ) :
        ...
        while api_retry_count < max_retries :
            try :
                ...
                error_tag = "响应超时"
                response_error = ResponseError( self.platform, self.model_name, exception = e,
                                                error_tag = error_tag )
    ...
    def _create_chat_completion( self, messages ) :
        ...
        if self.client is None :
            ...
            response_error = ResponseError( self.platform, self.model_name,
                                            exception = Exception( error_message ), error_tag = error_tag )
            raise response_error
    ...
    def _handle_response( self, response: Any, stream: bool, stream_real: bool, ) -> Tuple[ Any, int ] :
        """处理响应"""

        # 非流式响应，直接获取 content
        if isinstance( response, str ) :
            error_tag = "response类型是字符串"
            response_error = ResponseError( self.platform, self.model_name, exception = Exception( response ),
                                            error_tag = error_tag )
            raise response_error
...
class BaseOpenai( BaseClient ) :
    ...
    def __init__( self, platform, base_url, api_keys, model_name, stream = False, stream_real = False,
                  extra_body = None ) :
    ...
    def _init_client( self ) :
        ...
        if not self.api_keys :
            ...
            response_error = ResponseError( self.platform, self.model_name, exception = Exception( error_message ),
                                            error_tag = error_tag )
            raise response_error
